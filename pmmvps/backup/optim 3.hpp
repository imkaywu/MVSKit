...
public:
    ...
    void computeUnits(const Patch& patch, vector<int>& indexes, vector<float>& units, vector<Vector4f, Eigen::aligned_allocator<Vector4f> >& rays) const;
    ...
    // get texture from all visible viewpoints, and all illumination conditions
    ...
    // leave it for now
    // compute tangents of a patch
    void computeTangents(const Vector4f& coord, const int image, Vector4f& tx, Vector4f& ty);
    // determine the type of derivative
    void dxdy(const Vector3f icoord, e_di dx, e_di dy);
    // find the normal in the neighbourhood
    void findNormals(const int image, const Vector4f& coord, vector<Vector4f, Eigen::aligned_allocator<Vector4f> >& normals);
...
protected:
    ...
    vector<vector<Vector3f> > m_texs; // m_nviews x (wsize*wsize)
    vector<vector<vector<Vector3f> > > m_textures; // m_nviews x m_nillums x (wsize*wsize)
    vector<Vector3f> m_colors; // m_nviews x m_nillums
    vector<Vector3f> m_tans; // tangents of the surface
    // Weights
