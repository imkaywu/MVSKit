...
public:
    ...
    // Image related
    //-----------------------------------------------------------------
    // In perspective projection, we have (from UofT's slides)
    // x = (f_x / p_z) * p_x, and
    // y = (f_y / p_z) * p_y
    // we can get the 3D coord (p_x, p_y, p_z) from pixel coord (x, y, f)
    // p_x = (p_z / f_x) * x
    // p_y = (p_z / f_y) * y
    // since f_x and f_y are mostly very close, we have
    // (p_z / f_x) = (p_z / f_y) = (2 * p_z / f_x + f_y)
    // fz: the distance between the patch center and CoP, probably should be renamed as dz
...
protected:
    ...
    // image plane scale, sum of focal lengths
    ...
    vector<vector<vector<Vector3f> > > m_textures; // m_nimages x m_nillums x (wsize*wsize)
    // Weights
