...
void Optim::constraintImages(Patch& patch, const float nccThreshold) {
    ...
    if (patch.m_images.size() < m_pmmvps.m_tau) {
        return;
    }
...
//    const float angle2 = xs[2] * m_inst->m_ascale;
    ...
    const int nillums = m_inst->m_pmmvps.m_nillums;
    
    double cost = 0.0;
    const double bias = 0.0f;
    
    // decode to get coord, and normal
    ...
    // compute the photo-consistency cost
    ...
    // compute the orientation-consistency cost
//    Vector4f tx, ty;
//    m_inst->computeTangents(coord, index, tx, ty);
//    cost = m_inst->lambda * cost + (1 - m_inst->lambda) * (powf(tx.dot(normal), 2.0f) + powf(ty.dot(normal), 2.0f));
...
float Optim::computeINCC(const Vector4f& coord, const Vector4f& normal, const vector<int> indexes, const Vector4f& pxaxis, const Vector4f& pyaxis, const int isRobust) {
    ...
    for (int i = 1; i < sz; ++i) {
        if (!texs[i].empty()) {
            ...
            for (int illum = 0; illum < m_pmmvps.m_nillums; ++illum) {
                if (isRobust) {
//                    cerr << dot(texs[0][illum], texs[i][illum]) << endl;
    ...
    for (int illum = 0; illum < m_pmmvps.m_nillums; ++illum) {
        ...
        for (int y = 0; y < size; ++y) {
            ...
            for (int x = 0; x < size; ++x) {
                Vector3f color = m_pmmvps.m_photoSet.getColor(index, samp(0), samp(1), newLevel, illum);
                int ind = y * size + x;
                texs[illum][ind] = color;
                samp += dx;
            }
        }
    }
...
int Optim::getTexSafe(const int index, const int size, const Vector3f& center, const Vector3f& dx, const Vector3f& dy, const int level) const {
    ...
    const int margin2 = 1; // 1 should be enough;
